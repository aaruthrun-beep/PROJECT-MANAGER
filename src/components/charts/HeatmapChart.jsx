    ...
    <div className="glass rounded-2xl p-6 border border-white/10">
              ...
              {week.map((day, di) => (
                day ? (
                  <Tooltip key={di} content={`${day.count} entries on ${day.day}`}>
                    <div className={`w-3 h-3 rounded-sm ${getColor(day.count)} hover:ring-1 hover:ring-indigo-400 transition-all cursor-pointer`} />
                  </Tooltip>
                ) : (
      ...
      <div className="flex items-center gap-1 mt-3 justify-end">
        ...
        {[0, 0.2, 0.4, 0.6, 0.8].map((v, i) => (
          <div key={i} className={`w-3 h-3 rounded-sm ${v === 0 ? 'bg-white/[0.03]' : `bg-indigo-500/${Math.round(v * 100)}`}`} />
        ))}
