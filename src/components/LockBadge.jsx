...
export default function LockBadge() {
  const { isOwner, isSignedIn, loading, login } = useAuth()

  if (loading) return <div className="w-8 h-8 rounded-xl bg-white/5 skeleton-pulse" />

  if (!isSignedIn) {
    return (
      <button onClick={login} className="p-2 rounded-xl text-zinc-500 hover:text-white hover:bg-white/[0.03] transition-all" title="Sign in">
      ...
      <div className="relative group">
        <UserButton
          appearance={{
            elements: {
              rootBox: 'scale-90',
              avatarBox: 'w-8 h-8 rounded-xl border-2 border-zinc-600/30',
          ...
          afterSignOutUrl="/"
        />
        <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-red-500 border-2 border-black" title="Read-only view" />
      </div>
    )
  }
