  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">View your daily log entries by date</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card hover={false}>
            <div className="flex items-center justify-between mb-6">
              <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 rounded-lg hover:bg-white/5 text-gray-400 hover:text-white transition-all">
                <ChevronLeft size={20} />
              </button>
              <h3 className="text-lg font-semibold text-white">{format(currentMonth, 'MMMM yyyy')}</h3>
              <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 rounded-lg hover:bg-white/5 text-gray-400 hover:text-white transition-all">
        ...
        <div>
          <Card hover={false}>
            ...
            {dayEntries.length === 0 ? (
              <div className="text-center py-8 text-gray-500 text-sm">No entries for this day</div>
            ) : (
              <div className="space-y-3">
                {dayEntries.map(e => {
                  const project = data.projects.find(p => p.id === e.projectId)
                  return (
                    <div key={e.id} className="glass rounded-xl p-3 border border-white/10">
                      ...
                      {(e.images?.length > 0 || e.videos?.length > 0) && (
                        <p className="text-[10px] text-purple-400 mt-1">{e.images?.length || 0} photos, {e.videos?.length || 0} videos</p>
                      )}
                    </div>
                  )
                })}
