...
const statusColors = {
  ...
  archived: 'bg-gray-500/20 text-zinc-400 border-gray-500/30',
}
...
const priorityColors = {
  ...
  critical: 'border-l-amber-600/50',
}
  ...
  return (
    <div ref={tilt.ref} onMouseMove={tilt.handleMouseMove} onMouseLeave={tilt.handleMouseLeave}
      ...
      <Link to={`/projects/${project.id}`}
        className={`block glass rounded-2xl p-5 border border-zinc-700/60 border-l-4 hover:border-amber-600/30 hover:bg-white/[0.07] transition-all shine ${priorityColors[project.priority] || priorityColors.medium}`}
        ...
        <div className="flex items-start justify-between mb-3" style={{ transformStyle: 'preserve-3d' }}>
          <div className="flex-1 min-w-0" style={{ transform: 'translateZ(30px)' }}>
            <h3 className="text-base font-semibold text-white group-hover:text-amber-500 transition-colors truncate">
              {project.name}
            </h3>
            <p className="text-xs text-zinc-400 mt-0.5 line-clamp-2">{project.description}</p>
          </div>
          <ExternalLink size={16} className="text-zinc-600 group-hover:text-amber-500 shrink-0 mt-0.5 ml-2" style={{ transform: 'translateZ(20px)' }} />
        </div>
        <div className="flex items-center gap-2 text-[11px] text-zinc-500" style={{ transform: 'translateZ(15px)' }}>
      ...
      {onDelete && (
        <button onClick={(e) => { e.preventDefault(); onDelete(project.id) }}
          className="absolute bottom-2 right-2 p-1.5 rounded-lg bg-red-500/10 text-red-400 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 transition-all" style={{ transform: 'translateZ(25px)' }}>
          <Trash2 size={14} />
        </button>
      )}
    </div>
