  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(true)}
        className="md:hidden p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-800/80 active:scale-95 transition-all">
        <Search size={18} />
      </button>

      {/* Desktop: search bar */}
      <button onClick={() => setOpen(true)}
        className="hidden md:flex items-center gap-2 glass rounded-xl px-3 py-2 text-sm text-zinc-400 hover:text-white hover:bg-white/[0.08] transition-all w-full max-w-md group">
        <Search size={15} className="shrink-0" />
        <span className="flex-1 text-left">Search projects, entries...</span>
        <kbd className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-zinc-500 font-mono shrink-0">Ctrl+K</kbd>
      </button>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 border-b border-white/5">
                ...
                <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)}
                  ...
                  className="flex-1 bg-transparent text-white placeholder-zinc-500 focus:outline-none text-sm sm:text-base" />
                {query && <button onClick={() => setQuery('')} className="text-zinc-500 hover:text-white p-1"><X size={16} /></button>}
              </div>
              <div className="flex-1 overflow-y-auto p-2 sm:p-3">
                ...
                {results.projects.length > 0 && (
                  <div className="mb-2">
                    <p className="text-[10px] text-zinc-500 px-3 py-1.5 font-medium uppercase tracking-wider">Projects</p>
                    {results.projects.map(p => (
                      <button key={p.id} onClick={() => { navigate(`/projects/${p.id}`); setOpen(false) }}
                        ...
                        <FolderKanban size={15} className="text-amber-500 shrink-0" />
                ...
                {results.entries.length > 0 && (
                  <div className="mb-2">
                    <p className="text-[10px] text-zinc-500 px-3 py-1.5 font-medium uppercase tracking-wider">Log Entries</p>
                    {results.entries.map(e => (
                      <button key={e.id} onClick={() => { navigate(`/daily-log#log-${e.id}`); setOpen(false) }}
                        className="w-full flex items-center gap-2 sm:gap-3 px-3 py-2.5 rounded-xl hover:bg-zinc-800/80 text-left transition-all active:scale-[0.98]">
                        <CalendarPlus size={15} className="text-emerald-400 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-sm truncate">{e.title}</p>
                          <p className="text-zinc-500 text-xs truncate">{e.content}</p>
                        </div>
                        <ArrowRight size={14} className="text-zinc-600 shrink-0" />
                      </button>
                    ))}
                ...
                {results.milestones.length > 0 && (
                  <div>
                    <p className="text-[10px] text-zinc-500 px-3 py-1.5 font-medium uppercase tracking-wider">Milestones</p>
              ...
              <div className="p-2 sm:p-3 border-t border-white/5 text-center hidden sm:block">
                <p className="text-[10px] text-zinc-600">
                  <kbd className="px-1 py-0.5 rounded bg-white/10 text-zinc-500 mx-0.5">Esc</kbd> Close
                </p>
              </div>
            </motion.div>
          </div>
