    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-white flex items-center gap-3">
          <BarChart3 size={24} className="text-amber-500" /> Analytics
        </h2>
        <p className="text-zinc-400 mt-1">Insights and metrics for your productivity</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Entries', value: stats.totalEntries || 0, icon: Calendar, color: 'text-amber-500 bg-amber-600/10' },
          ...
          { label: 'Media Items', value: entriesWithMedia, icon: Activity, color: 'text-amber-500 bg-amber-600/10' },
        ].map((s, i) => {
          const Icon = s.icon
          return (
            <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="glass rounded-2xl p-4 border border-zinc-700/60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card>
          ...
          {priorityData.length === 0 ? (
            <p className="text-zinc-500 text-sm py-8 text-center">No projects yet</p>
          ) : (
            <ResponsiveContainer width="100%" height={chartHeight}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {insights.map((insight, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
              className="glass rounded-xl p-3 border border-zinc-700/60 flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-amber-500 shrink-0" />
              <p className="text-sm text-zinc-300">{insight}</p>
            </motion.div>
          ))}
          {insights.length === 0 && <p className="text-zinc-500 text-sm col-span-2 py-4 text-center">Add some data to see insights</p>}
        </div>
      </Card>
    </motion.div>
  )
