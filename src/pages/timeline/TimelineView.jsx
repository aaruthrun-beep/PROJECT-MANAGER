    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-white flex items-center gap-3">
          <Radio size={24} className="text-indigo-400" /> Timeline
        </h2>
        <p className="text-gray-400 mt-1">All your activity in chronological order</p>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {[
          ...
          <button key={f.value} onClick={() => setFilter(f.value)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === f.value ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'glass text-gray-400 hover:text-white border border-white/10'}`}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-8 relative">
        ...
        {Object.entries(grouped).map(([date, items]) => (
          <div key={date}>
            <div className="flex items-center gap-3 mb-4 sticky top-0 bg-gray-950/80 backdrop-blur-sm z-10 py-2">
                  ...
                  <motion.div key={`${event.type}-${event.title}-${i}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {event.description && <p className="text-gray-400 text-xs line-clamp-2">{event.description}</p>}
                        {event.project && <p className="text-[10px] text-indigo-400 mt-1">{event.project}</p>}
                      </div>
                      <span className="text-xs text-gray-500 shrink-0">{format(event.date, 'HH:mm')}</span>
                    </div>
                  </motion.div>
                )
              })}
        ...
        {filtered.length === 0 && (
          <Card className="text-center py-12">
            <Radio size={32} className="mx-auto mb-2 text-gray-500/50" />
