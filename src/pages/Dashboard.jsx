...
export default function Dashboard() {
  ...
  const statCards = [
    { label: 'Total Projects', value: stats.totalProjects || 0, icon: FolderKanban, color: 'from-indigo-500/20 to-purple-500/20 border-indigo-500/30', iconBg: 'bg-indigo-500/20 text-indigo-300', suffix: '' },
    ...
    { label: "Today's Logs", value: stats.entriesToday || 0, icon: TrendingUp, color: 'from-pink-500/20 to-rose-500/20 border-pink-500/30', iconBg: 'bg-pink-500/20 text-pink-300', suffix: '' },
    ...
    { label: 'Milestones', value: `${stats.completedMilestones || 0}/${stats.totalMilestones || 0}`, icon: Target, color: 'from-violet-500/20 to-purple-500/20 border-violet-500/30', iconBg: 'bg-violet-500/20 text-violet-300', suffix: '' },
  ]
      ...
      <ScrollReveal>
        <div className="mb-8 flex items-center justify-between">
          ...
          <button onClick={() => setConfetti(true)}
            className="hidden md:flex items-center gap-2 glass px-4 py-2 rounded-xl text-sm text-gray-400 hover:text-white border border-white/10 hover:border-indigo-500/30 transition-all">
                ...
                return (
                  <motion.div key={e.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45 + i * 0.05 }}>
                    <Card hover={false}>
                      ...
                      {project && <span className="inline-flex px-2 py-0.5 text-[10px] rounded-lg bg-indigo-500/20 text-indigo-400 border border-indigo-500/30">{project.name}</span>}
                    </Card>
                  </motion.div>
                )
              })
